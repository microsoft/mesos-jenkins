...
function Install-Prerequisites {
    ...
    foreach($program in $prerequisites.Keys) {
        ...
        #  consider using Start-ExecuteWithRetry
...
function Set-LatestDiagnosticsCommit {
    ...
    try {
        ...
        Start-ExternalCommand { git.exe log -n 1 } -ErrorMessage "Failed to get the latest commit message for the Mesos git repo" | Out-File "$DIAGNOSTICS_BUILD_LOGS_DIR\latest-commit.log"
...
function Build-DiagnosticsBinaries {
    ...
    write-output "DiagnosticsBinaries location: $($currLoc.Path)" 
    $logsUrl = Get-BuildLogsUrl
    try {
...
try {
    ...
    # The following upload does not work for me. I am suspecting it requires some privilege.
    # For testing other part of the code, I replace it with simple file copying below.
